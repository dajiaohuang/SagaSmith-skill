...
from .user_context import read_player_roles, write_player_roles
from ..modules.search import ModuleSearchService
from ..rules.ingest import RuleIngestService
from ..rules.search import RuleSearchService
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m saga_domain.cli")
...
def _vector_status(store: VectorStore) -> dict[str, Any]:
    ...
    return {
        ...
        "embedding": {
            ...
            "note": (
                "Model choices are optional and are not forced by detected hardware. "
...
def _vector_reindex(database: Database, store: VectorStore) -> dict[str, int]:
    ...
    from ..rules.ingest import _ensure_embedding_model

    if not store.enabled:
        raise RuntimeError("ChromaDB is not enabled")
    for base_name in ("dnd_rules", "dnd_modules"):
        for profile in configured_profiles():
            store.drop_collection(collection_name(base_name, profile))

    results = {"dnd_rules": 0, "dnd_modules": 0}
    with database.transaction() as session:
        rule_rows = list(
            session.execute(
        ...
        for chunk, source in module_rows:
            module_texts[source.id] = (
                module_texts.get(source.id, "") + f"\n{chunk.chunk_text}"
            )
